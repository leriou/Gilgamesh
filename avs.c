...
#define W(o)            \
    c    = C(x, y) + o; \
    c[0] = 255;         \
    c[1] = c[2] = d % 512 < 256 ? d % 512 : 511 - d % 512
#define V(x, y) \
    (x) >= 0 && (x) < w && (y) >= 0 && (y) < h && !*C(x, y) && m(x, y)

static const int w = 256, h = 256, a = w * 2 + 1, b = h * 2 + 1;
static unsigned char img[a * b * 3], *c;

int m(int x, int y) {
    float u = 2.4f * x / w - 1.2f, v = -2.4f * y / h + 1.3f,
          w = u * u + v * v - 1;
...
void f(int x, int y, int d) {
    ...
    for (int j = rand() % 24 * 4, i = 0; i < 4; i++)
        switch (
            "ABCDABDCACBDACDBADBCADCBBACDBADCBCADBCDABDACBDCACABDCADBCBADCBDACD"
            "ABCDBADABCDACBDBACDBCADCABDCBA"[i + j]) {
            case 'A':
                if (V(x - 1, y)) {
                    W(-3);
                    f(x - 1, y, d + 1);
